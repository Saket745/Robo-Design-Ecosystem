/**
 * Shared YAML parser (zero dependencies).
 * Parses basic YAML structures, handles arrays, objects, booleans, numbers, and comments.
 *
 * @param {string} content YAML string content to parse
 * @returns {object} Parsed Javascript object representation
 */
      ...
      if (trimmed.startsWith('-')) {
        ...
        if (valStr.includes(':')) {
          ...
          if (parsedVal.startsWith('[') && parsedVal.endsWith(']')) {
            const inner = parsedVal.substring(1, parsedVal.length - 1).trim();
            parsedVal = inner ? inner.split(',').map(s => s.trim().replace(/^['"]|['"]$/g, '')) : [];
