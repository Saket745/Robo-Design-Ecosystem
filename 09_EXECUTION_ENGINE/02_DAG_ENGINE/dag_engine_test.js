...
test('OOP DAGEngine - addNode should use empty array as default dependencies if not provided', () => {
  const engine = new DAGEngine();
  engine.addNode('A');

  assert.strictEqual(engine.nodes.size, 1);
  assert.ok(engine.nodes.has('A'));
  assert.strictEqual(engine.nodes.get('A').size, 0);
});

test('OOP DAGEngine - addNode should preserve existing nodes and their dependencies when referenced as dependencies', () => {
  const engine = new DAGEngine();
  engine.addNode('A', ['X']);
  assert.strictEqual(engine.nodes.get('A').size, 1);
  assert.ok(engine.nodes.get('A').has('X'));

  // Add B which depends on A. The existing dependencies of A should be preserved.
  engine.addNode('B', ['A']);
  assert.strictEqual(engine.nodes.get('A').size, 1);
  assert.ok(engine.nodes.get('A').has('X'));
});

test('OOP DAGEngine - addNode should append dependencies to an already existing node when called again', () => {
  const engine = new DAGEngine();
  engine.addNode('A', ['B']);
  assert.strictEqual(engine.nodes.get('A').size, 1);
  assert.ok(engine.nodes.get('A').has('B'));

  // Call addNode again on A with different dependencies
  engine.addNode('A', ['C', 'D']);
  const aDeps = engine.nodes.get('A');
  assert.strictEqual(aDeps.size, 3);
  assert.ok(aDeps.has('B'));
  assert.ok(aDeps.has('C'));
  assert.ok(aDeps.has('D'));
});

test('OOP DAGEngine - addNode should handle duplicate dependencies in input array gracefully', () => {
  const engine = new DAGEngine();
  engine.addNode('A', ['B', 'B', 'C']);
